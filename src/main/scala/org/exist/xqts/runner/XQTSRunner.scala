      ...
      case Right(localXqtsDir) =>
        ...
        ExistServer.start() match {
          case Right(server) =>
            ...
            val xqtsRunner = system.actorOf(Props(classOf[XQTSRunnerActor], settings.xmlParserBufferSize, server, parserActorClass, serializerActorClass, styleDir, cmdConfig.outputDir.getOrElse(Paths.get(settings.outputDir)), None), name = "XQTSRunner")
  ...
  private def ensureXqtsPresent(xqtsVersion: XQTSVersion, settings: SettingsImpl, overrideLocalDir: Option[Path]): Either[Throwable, Path] = {
    ...
    settings.xqtsVersions.get(XQTSVersion.toVersionName(xqtsVersion)) match {
      ...
      case None =>
        ...
        Left(new IllegalArgumentException(s"We only support XQTS versions $supported, but version: ${XQTSVersion.label(xqtsVersion)} was requested"))
    }
  }
}
