...
#
# Options:
#   --xqts-version VERSION   3.1 or HEAD (default: HEAD)
#   --batch-size N           test sets per batch (default: 50)
#   --heap SIZE              JVM heap size (default: 4g)
#   --timeout SECS           per-batch timeout in seconds (default: 300)
...
#
# Environment:
#   JAVA_HOME  JDK to run the runner JAR with (falls back to `java` on PATH)
#   PARSER     eXist XQuery parser to select via -Dexist.parser (default: antlr2)
#
# Examples:
#   ./run-batched.sh --xqts-version HEAD --batch-size 40 --heap 6g
#   ./run-batched.sh --xqts-version 3.1 --resume
#   ./run-batched.sh --xqts-version HEAD --test-set-pattern 'fn-.*' --batch-size 30
...
# Use $JAVA_HOME's JDK if set, otherwise fall back to whatever is on PATH.
...
run_batch() {
  ...
  local cmd=("$JAVA_BIN" "-Xmx${HEAP}" "-XX:+ExitOnOutOfMemoryError"
  ...
  (
    ...
    if kill -0 $batch_pid 2>/dev/null; then
      ...
      if [[ -n "$java_pid" ]]; then
        "$JSTACK_BIN" "$java_pid" > "$jstack_file" 2>&1 || true
